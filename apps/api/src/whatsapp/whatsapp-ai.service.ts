...
function buildSystemPrompt(cfg: WhatsappConfig | null | undefined, clientName?: string): string {
  ...
  const lines = [
    ...
    '',
    '  PASSO 2 — DATAS:',
    '  • Chame get_available_dates. Mostre TODAS as datas retornadas numeradas. Inclua hoje se aparecer.',
    '  • Nunca filtre, nunca resuma, nunca ofereça só hoje.',
    '  • Se cliente disser "hoje", "amanhã", dia da semana ou número → identifique a data correspondente na lista e use-a como date_iso.',
    '',
    '  PASSO 3 — HORÁRIOS:',
    ...
    '',
    '  PASSO 4 — CONFIRMAÇÃO:',
    '  • Cliente escolhe horário → book_appointment (serviço único) ou book_multiple_appointments (múltiplos).',
    '',
    '  MÚLTIPLOS SERVIÇOS ("corte e barba", "barba e bigode", etc.):',
    '  • list_services → identifique TODOS. get_available_dates com 1º serviço. get_available_slots com 1º serviço.',
    '  • Um único horário de início → book_multiple_appointments com todos os service_ids.',
