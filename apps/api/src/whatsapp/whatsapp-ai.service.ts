...
function buildSystemPrompt(cfg: WhatsappConfig | null | undefined, clientName?: string): string {
  ...
  const lines = [
    ...
    '  1. list_services → identifique TODOS os serviços mencionados pelo nome e guarde TODOS os IDs.',
    '  2. Se cliente já disse a data ("hoje", "amanhã", dia da semana):',
    '     → converta para date_iso e chame get_available_slots com o 1º service_id e essa data.',
    '     Se cliente NÃO disse a data:',
    '     → chame get_available_dates com o 1º service_id, mostre as datas, pergunte qual prefere.',
    '     → Quando responder, chame get_available_slots com o 1º service_id e a data escolhida.',
    '  3. Mostre os horários disponíveis. Pergunte: "Qual horário você prefere para começar?"',
    '  4. Cliente responde com horário → chame book_multiple_appointments com TODOS os service_ids, a data e o horário.',
    '  ⛔ NUNCA chame book_appointment para múltiplos serviços.',
    '  ⛔ NUNCA peça horário separado para cada serviço.',
    '  ⛔ NUNCA pergunte "qual serviço primeiro?" — use a ordem que o cliente mencionou.',
    '',
    '━━ FLUXO SERVIÇO ÚNICO ━━',
    '  PASSO 1 — SERVIÇO:',
    '  • Se cliente mencionou serviço → list_services, identifique pelo nome, vá para PASSO 2.',
    '  • Se NÃO mencionou → list_services, mostre lista numerada IMEDIATAMENTE. Não pergunte se quer ver. Pergunte ao final: "Qual serviço você deseja?"',
    '',
    '  PASSO 2 — DATAS:',
    '  • Se cliente já disse a data → converta para date_iso e vá direto ao PASSO 3.',
    '  • Se não disse → chame get_available_dates. Mostre TODAS as datas retornadas numeradas.',
    '  • Se cliente disser "hoje", "amanhã", dia da semana ou número → identifique a data correspondente e use como date_iso.',
    '',
    '  PASSO 3 — HORÁRIOS:',
    '  • Chame get_available_slots com a date_iso correta.',
    '  • Mostre cada horário em linha separada: "1 - 08:00", "2 - 09:00", etc.',
    '  • NUNCA gere texto aleatório, binário ou codificado. Apenas os horários da ferramenta.',
    '',
    '  PASSO 4 — CONFIRMAÇÃO:',
    '  • Cliente escolhe horário → book_appointment.',
    '',
