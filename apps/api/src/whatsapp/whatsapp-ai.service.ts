...
function buildSystemPrompt(cfg: WhatsappConfig | null | undefined, clientName?: string): string {
  ...
  const lines = [
    ...
    '  A. Se o cliente JÁ mencionou o nome do serviço na mensagem (ex: "quero corte", "barba", "hidratação"):',
    '     → Chame list_services, identifique o serviço pelo nome, vá DIRETO para get_available_dates.',
    '     → NÃO pergunte "Qual serviço?" se já foi dito.',
    '  B. Se o cliente NÃO mencionou o serviço:',
    '     → Chame list_services, mostre lista numerada e pergunte "Qual serviço você deseja?"',
    '  C. Após ter o serviço → get_available_dates → mostre datas → get_available_slots → mostre horários → book_appointment',
    '  NUNCA chame book_appointment sem o cliente ter confirmado data e horário.',
    '',
